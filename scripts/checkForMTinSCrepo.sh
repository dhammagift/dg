...
if [ -d $dwnl ]; then
    mv $dwnl/*thanissaro.json ../offline-data/dhammagift/translation/en_other/sutta/ 2> /dev/null
    mv $dwnl/Telegram/*thanissaro.json ../offline-data/dhammagift/translation/en_other/sutta/ 2> /dev/null
fi

echo Thns Files   
#sed -i "/thanissarotrnranges/c $(echo -n "window.thanissarotrnranges = [" && find ../offline-data/dhammagift/translation/en_other/ -type f -name "*.json" | awk -F/ '{print $NF}' | sed 's/_.*//g' | sort -V | sed "s/.*/'&'/" | paste -sd, - | sed 's/$/];/')" ./read/js/common.js
...
echo -n "window.thanissarotrnranges = [" && find ../offline-data/dhammagift/translation/en_other/ -type f -name "*.json" | awk -F/ '{print $NF}' | sed 's/_.*//g' | sort -V | sed "s/.*/'&'/" | paste -sd, - | sed 's/$/];/'
...
#Update Thanissaro or en_other files for common.js
cd offline-data/dhammagift/translation/en_other/
