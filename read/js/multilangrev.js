...
async function buildSutta(slug) {
  ...
  Promise.all([rootResponse, translationResponse, engtranslationResponse, htmlResponse, varResponse]).then(responses => {
      ...
      // Шаг 1: Формируем ПРЯМОЙ валидный HTML, но с перевернутыми СЛОВАМИ внутри
      ...
      if (finder && finder.trim() !== "") {
        ...
        if (varData[segment]) varData[segment] = varData[segment].replace(regex, highlight);
          ...
          // Очищаем и переворачиваем по словам
      ...
      function reverseDOMTree(element) {
          // Разворачиваем все дочерние узлы (включая текстовые пробелы)
          ...
          // Рекурсивно идем вглубь элементов
      ...
      let htmlContent = tempDiv.innerHTML; // Идеально перевернутый текст и структура
      ...
      let dUrl = mlUrl.replace("/rev/", "/d");
      let frUrl = mlUrl.replace("/rev/", "/frev/");
