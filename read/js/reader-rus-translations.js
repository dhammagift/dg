...
//   onclick="alert(this.getAttribute('data-slug'))"
      ...
      if (typeof tbwLinksData !== 'undefined') {
          ...
          if (hasTbw) {
              // Кнопка BB всегда локальная
              bbLinkHtml = `&nbsp;<a target="" title="BB and Other translations" href="/b/?q=${slug}">BB</a>`;
              
              // Кнопка TBW: ВСЕГДА ОФФЛАЙН (локальный путь /bw/)
              const bookMatch = slug.match(/^[a-z]+/); 
              const book = bookMatch ? bookMatch[0] : "";
              const tbwUrl = `/bw/${book}/${slug}.html`; 
                  
              tbwLinkHtml = `&nbsp;<a target="_blank" title="TheBuddhasWords.net (Offline Copy)" href="${tbwUrl}">TBW</a>`;
          }
      }
      ...
      if (typeof thruLinksData !== 'undefined') {
          ...
          if (ruItem) {
              // ВСЕГДА ОФФЛАЙН (локальный путь)
              const ruUrl = `/theravada.ru/Teaching/Canon/Suttanta/Texts/${ruItem[1]}`;
              ruLinkHtml = `&nbsp;<a title="Theravada.ru (Offline Copy)" target="_blank" href="${ruUrl}">Th.ru</a>`;
          }
      }

      // 3. Проверяем Theravada.su (Th.su) - ЗДЕСЬ ОСТАВЛЯЕМ ПРОВЕРКУ
      let suLinkHtml = "";
      if (isLocal) {
      ...
      $.ajax({
        ...
        let nextName = nextArray.slice(1).join(" ").replace(/[0-9.]/g, '');

        let nextPrint = (nextName === undefined || nextName.trim() === "") ? nextSlugPrint : `${nextSlugPrint} <span class="sutta-name"> ${nextName.trim()}</span>`;
        let finder = (params.get("s") || "").replace(/ṃ/g, "ṁ");
        
        next.innerHTML = nextSlug ? `<a href="?q=${nextSlug}${params.has("s") ? `&s=${finder}` : ""}">${nextPrint.trim()}
      ...
      $.ajax({
        ...
        let prevName = prevArray.slice(1).join(" ").replace(/[0-9.]/g, '');

        let prevPrint = (prevName === undefined || prevName.trim() === "") ? prevSlugPrint : `${prevSlugPrint} <span class="sutta-name"> ${prevName.trim()}</span>`;
        let finder = (params.get("s") || "").replace(/ṃ/g, "ṁ");
        
        previous.innerHTML = prevSlug ? `<a href="?q=${prevSlug}${params.has("s") ? `&s=${finder}` : ""}">
          <svg xmlns="http://www.w3.org/2000/svg" id="body_1" width="15" height="11">
