...
function runMeditationCycle() {
    ...
    window.memoRestartTimeout = setTimeout(() => {
        ...
        // ЗВУЧИТ ГОНГ (Конец основной сессии)
        if (soundChoice !== 'none') {
            new Audio(`/assets/sounds/${soundChoice}`).play().catch(e => console.warn(e));
        }
...
function startMemoTTS(isRestart = false) {
    ...
    if (!container.innerHTML) {
        ...
        // Игнорируем URL и отдаем настройки строго по языку текста
