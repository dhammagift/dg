...
async function addToSearchHistory() {
    try {
        ...
        saveToHistory(key, url);

        // Попытка улучшить ключ (асинхронно)
        if (/\d/.test(key)) {
            tryEnhanceKey(key, url).catch(console.debug);
        }
    } catch (e) {
...
// Безопасная обработка поискового запроса
...
// Попытка добавить название сутты
async function tryEnhanceKey(key, url) {
    // Пропускаем если уже содержит название
    ...
    if (suttaName) {
        saveToHistory(`${baseKey} ${suttaName}`, url);
    }
}

// Загрузка данных с обработкой всех возможных форматов
async function loadTextData() {
    // 1. Проверяем глобальную переменную
    if (typeof textinfo !== 'undefined') return textinfo;

    // 2. Пробуем загрузить как модуль
    try {
        const module = await import('/assets/js/textinfo.js?update=' + Date.now());
        if (module.textinfo) return module.textinfo;
    } catch {}

    // 3. Пробуем загрузить как сырой текст и преобразовать
    ...
    try {
        ...
        try {
            ...
            console.error("Не удалось загрузить данные сутт");
            return {};
        }
    }
}

// Сохранение в историю (обновлённая версия)
// Сохранение в историю (финальная версия)
function saveToHistory(key, url) {
    const value = url.pathname + url.search + url.hash;
    const timestamp = new Date().toISOString();
    
    let history = JSON.parse(localStorage.getItem("localSearchHistory")) || [];
    
    // Проверяем, является ли ключ номером сутты (содержит цифры)
    const isSuttaNumber = /\d/.test(key);
    let baseKey = key;
    
    if (isSuttaNumber) {
        baseKey = key.split(/\s+/)[0]; // Получаем базовый ключ (без названия)
        
        // Ищем существующую запись с таким же базовым ключом
        const existingIndex = history.findIndex(([k]) => {
        ...
        if (existingIndex >= 0) {
            const [existingKey] = history[existingIndex];
            
            // Определяем, нужно ли обновлять запись
            const shouldUpdate = 
                (key.includes(' ') && !existingKey.includes(' ')) ||  // Новая имеет название
                (key.includes(' ') === existingKey.includes(' '));    // Обе одинаковые
            
            if (shouldUpdate) {
                // Удаляем старую запись
                history.splice(existingIndex, 1);
                // Добавляем новую в начало (с актуальным timestamp)
    ...
    // Для новых записей или если не нужно обновлять
    // Удаляем существующую запись с таким же ключом (если есть)
