...
if (typeof quickModalIsOpen !== 'undefined' && quickModalIsOpen) {
...
let isQuickModalRendered = false; // Флаг: собирали ли мы DOM
let quickModalIsOpen = false;     // Флаг: открыто ли сейчас окно
...
function toggleQuickModal() {
  // Ленивая инициализация DOM
  if (!isQuickModalRendered) {
    buildQuickModalDOM();
    isQuickModalRendered = true;
  }

  if (quickModalIsOpen) {
    // Закрываем, используя CSS классы
    quickOverlay.classList.remove("open");
    quickModal.classList.remove("open");
    quickModalIsOpen = false;
  } else {
    // Перед открытием обновляем данные, чтобы всегда была свежая история
    window.refreshQuickModalData();
...
// === ЛЕНИВАЯ ЗАГРУЗКА СЛОВАРЯ (ИСПРАВЛЕННАЯ) ===
window.isDictScriptLoaded = false;
let isDictLoading = false;

function loadDictionaryScript(callback) {
    if (window.isDictScriptLoaded) {
    ...
    if (typeof showBubbleNotification === 'function') {
        ...
        showBubbleNotification(isRu ? "Загрузка словаря..." : "Loading Dictionary...");
    }

    const script = document.createElement('script');
    script.src = "/assets/js/paliLookup.js";
    script.onload = () => {
        window.isDictScriptLoaded = true; // Железобетонный флаг
        isDictLoading = false;
...
// 1. Перехват кликов
document.addEventListener('click', function(e) {
    if (window.isDictScriptLoaded) return; // Если загружен - не вмешиваемся

    const isPaliWord = e.target.closest('.pli-lang, [lang="pi"]');
    const isDictBtn = e.target.closest('.toggle-dict-btn');

    if (isPaliWord || isDictBtn) {
        e.preventDefault();
        e.stopPropagation(); // Блокируем дальнейшее распространение клика

        // КРИТИЧНО: Сохраняем реальные координаты мыши!
        const clickX = e.clientX;
        const clickY = e.clientY;
        const target = e.target;

        loadDictionaryScript(() => {
            // Генерируем полноценный клик мыши с правильными координатами
...
}, true); // На фазе перехвата (capture)

// 2. Перехват горячих клавиш Alt+A и Alt+B
document.addEventListener("keydown", (event) => {
    if (window.isDictScriptLoaded) return;

    if (event.altKey && (event.code === "KeyA" || event.code === "KeyB")) {
        event.preventDefault();
        event.stopPropagation();
        
        // Сохраняем параметры нажатой клавиши
        const keyEventParams = {
            key: event.key, code: event.code, altKey: true, bubbles: true
        };

        loadDictionaryScript(() => {
            const keyEvent = new KeyboardEvent('keydown', keyEventParams);
